write-host "Enter the Env you want to work: "
			$env_new= read-host
...
if($env_new -Ceq "MyRepository")			
	{
			...
			{
				echo "Directory $path_backup does Not Exists"
				New-Item -Path $path_backup -ItemType directory 
				echo "Created Directory $path_backup"
			}
			else
			{
				echo "Already Exists"
...
if((Test-Path -Path "C:\Program Files\Microsoft Dynamics NAV\100" ) -or (Test-Path -Path "C:\Program Files (x86)\Microsoft Dynamics NAV\100" ))
{
    Import-Module "${env:ProgramFiles(x86)}\Microsoft Dynamics NAV\100\RoleTailored Client\Microsoft.Dynamics.Nav.Model.Tools.psd1" -force -DisableNameChecking
	Import-Module "C:\Program Files (x86)\Microsoft Dynamics NAV\100\RoleTailored Client\Microsoft.Dynamics.Nav.Model.Tools.psd1" -force -DisableNameChecking
    if(Test-Path -Path "C:\Program Files\Microsoft Dynamics NAV\100\Service")
	{
		Import-Module 'C:\Program Files\Microsoft Dynamics NAV\100\Service\NavAdminTool.ps1' -DisableNameChecking > $null
	}
	if( Test-Path -Path "C:\Program Files (x86)\Microsoft Dynamics NAV\100\Service" )
	{
		Import-Module 'C:\Program Files (x86)\Microsoft Dynamics NAV\100\Service\NavAdminTool.ps1' -DisableNameChecking > $null
	}
...
if (( $argument -eq "eundo" -or $argument -eq "EUNDO" ))
   {
	...
	if($argument -eq "update")
	{
		...
		cd C:\GITRepo
				...
				git  clone -q http://10.61.51.37:8080/tfs/Puma/ePumaGlobal/_git/NAV/ .
